...
class WebSocket 
{
	...
	{
        $write = $except = NULL;
        $sockets = array_column($this->sockets, 'resource');//從輸入數組中的單個列返回值,搜尋resource
        ...
        foreach ($sockets as $socket) {
			...
			{
                $client = socket_accept($this->master);//獲取read數組中活動的socket，並且把不活躍的從read數組中刪除
    ...
    private function dealMsg($socket, $recv_msg) 
	{
		...
		*/
		...
		$response['d_array'] = array_reverse($data_array);
